...
账户的 URI。 若其是加密领域的去中心化账户，请用 `profile.accounts[i].signature` 来核验所有权。若其是无法签名确认的中心化平台，用户必须将主账户的地址或域名（请见[RSS3 改进提案-2: 在册域名服务](./RIPs/RIP-2.md)）放于平台账户设置的某一位置以核验所有权。
