...
- [RFC 3986 - Uniform Resource Identifier (URI): Generic Syntax](https://datatracker.ietf.org/doc/html/rfc3986): The protocol uses a large number of RFC 3986 Uniform Resource Identifiers to identify things in RSS3. （[RFC 3986 - 统一资源标志符 (URI): 一般语法](https://datatracker.ietf.org/doc/html/rfc3986): 本协议使用了大量 RFC 3986 统一资源标志符来标识 RSS3 中的事项。）
...
当前文件的 URI （统一资源标志符）：若是在查询场景下需包含查询。
...
| 种类 | 是否签名 | URI | 列表元素 |
| --- | --- | --- | --- |
| 定制条目列表 | 真 | `CustomItemListURI` | `Item` |
| 聚合条目列表 | 假 | `AggregatedItemListURI` | `Item` |
| 定制链接列表 | 真 或 假 | `CustomLinkListURI` | `InstanceURI` |
| 聚合链接列表 | 假 | `AggregatedLinkListURI` | `InstanceURI` |
| 反向链接列表 | 假 | `BacklinkListURI` | `InstanceURI` |
...
- `account_platform` is defined by [RIP-1: Registered Account Platforms](./RIPs/RIP-1.md). （`account_platform` 由 [RIP-1: Registered Account Platforms](./RIPs/RIP-1.md) 定义。）
